...
{	 
	//Variable declaration
	 ...
	 static uint8_t temp_addr = 0x5c;   // I2C slave address for measuring temperature and humidity
	 static uint8_t humid_addr=0x23;		 // I2C slave address for measuring light
...
//SENSORS ---------------------
    ...
    case STATE_SEND1:
       ...
       twi_start((temp_addr<<1) + TWI_WRITE);
    ...
    case STATE_REC1:
      ...
      twi_start((temp_addr<<1) + TWI_READ);
	...
	case STATE_SEND2:
	  ...
	  //Starts i2c with 1st addr-->wanting to write
	  twi_start((humid_addr<<1) + TWI_WRITE);
	  //Writing 0x0 to the sensor with addr
	  twi_write(counter);
	  //Nothing else has to be sent
	  twi_stop();
	...
	case STATE_REC2:
	   //After giving the	order to the slave we want to read
	   twi_start((humid_addr<<1) + TWI_READ);
	   //Read the temperature integer part
	   result=twi_read_nack();
	   itoa(result,uart_string,10);
	   twi_stop();
